...
function includesUserEmail(str) {
    str = str.toLowerCase();
...
// Spammy term regex
const spamRegex = config.spamTerms &&
...
const FILTERS = [
    ...
    (msg) => {
        ...
        if (isAllCaps(name))
            msg.deny(`All caps (name)`);
        if (isAllCaps(address))
            msg.deny(`All caps (address)`);
    },
    (msg) => {
    ...
    (msg) => {
        ...
        if (name.length - sl.basicLatin > 0)
            msg.deny('non-latin chars (name)');
    },
    (msg) => {
    ...
    (msg) => {
        if (msg._.recipients.length <= 0)
            msg.deny('empty recipients');
    },
    (msg) => {
    ...
    (msg) => {
        ...
        if (/\d\d@gmail.com/.test(msg._.from))
            msg.deny('gmail## sender');
    },
    (msg) => {
        // Spam often sent to "foo###@gmail.com" addresses
        const suspect = msg._.recipients.filter((e) => /\d\d@gmail.com/.test(e.address));
        if (suspect.length >= 2)
            msg.deny('gmail## recipients');
    },
    (msg) => {
...
async function writeSubjects(subjects) {
    ...
    for (const [k, { time }] of Object.entries(subjects)) {
        if (time < now - SUBJECT_EXPIRY)
            delete subjects[k];
    }
    await fs.writeFile(getConfigPath(SUBJECTS_FILE), JSON.stringify(subjects, null, 2));
}
/**
 * Main entry point
 */
...
async function main() {
    ...
    if (ids.length > 0) {
        ...
        const filteredIds = await fetchAndFilter(imap, ids.map(String), (msg) => {
            ...
            const gMsg = Object.assign(Object.create(msg), {
                _allow: undefined,
                _deny: undefined,
                _: {
            ...
            if (!gMsg._allow)
                checkSubject(gMsg, spamIds);
            if (gMsg._deny) {
                if (!loggedTime)
                    logger.log(new Date().toLocaleString());
                loggedTime = true;
                logger.log(`${gMsg._deny}: (${gMsg._.from})${gMsg._.subject ? ` "${gMsg.subject}"` : ''}`);
            }
