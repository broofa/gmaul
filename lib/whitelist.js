...
class Addresses {
  static async load(filePath) {
    ...
    return new Addresses(JSON.parse(data))
  }
  ...
  lookup(email) {
    return this._addresses[email.toLowerCase()];
  }

  save(filePath) {
    // TODO: This should be atomic (write to temp then move to path)
    const json = JSON.stringify({addresses: this._addresses});
  ...
  markAddress(source, address, date) {
    ...
    if (!(address in this._addresses)) this._addresses[address] = {};
    const a = this._addresses[address];
